...
import { actGetMovieSearch } from "../redux/actions/actGetMovieSearch";

export const SearchMovie = () => {
  const navigate = useNavigate();
  const { query } = useParams();
  const [showSkeleton, setShowSkeleton] = useState(true);
  const search = useSelector((state) => state.getMovieSearch.movieSearch);
  const dispatch = useDispatch();

  useEffect(() => {
    const getMovieAll = async () => {
      await dispatch(actGetMovieSearch(query));
    };
    getMovieAll();
    if (search) {
      setTimeout(() => {
